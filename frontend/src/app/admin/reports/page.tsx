...
export default function ReportsPage() {
  ...
  const fetchReports = useCallback(async () => {
    ...
    try {
      ...
      const data: PaginatedResponse = await apiClient.get(`/admin/reports?${params}`)
      setReports(data.data)
      setTotalPages(data.last_page)
      setTotal(data.total)
    } catch (err) {
      showToast('error', err instanceof Error ? err.message : 'Failed to load reports')
    } finally {
      setLoading(false)
    }
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      showToast('success', (result as { message?: string }).message || 'Reports deleted')
      setSelected(new Set())
      fetchReports()
    } catch (err) {
  ...
  const handleBulkReprocess = async () => {
    ...
    try {
      ...
      showToast('success', (result as { message?: string }).message || 'Reports queued')
      setSelected(new Set())
      fetchReports()
    } catch (err) {
      showToast('error', err instanceof Error ? err.message : 'Bulk reprocess failed')
    } finally {
      setBulkLoading(false)
    }
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-500">You need the <code className="bg-gray-100 px-2 py-0.5 rounded text-sm">manage_reports</code> permission.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-500 mt-1">{total} total report{total !== 1 ? 's' : ''}. Filter, inspect, and manage in bulk.</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2">
          <select
            ...
            className="px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white"
          >
      ...
      {selected.size > 0 && (
        <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-3 flex items-center justify-between">
          <span className="text-sm text-indigo-700 font-medium">{selected.size} report(s) selected</span>
          <div className="flex gap-2">
            <button
        ...
        {loading ? (
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              {reports.map((r) => (
                <tr key={r.id} className={`hover:bg-gray-50/50 transition-colors ${selected.has(r.id) ? 'bg-indigo-50/30' : ''}`}>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-900 font-medium max-w-[200px] truncate" title={r.original_filename}>
                    {r.original_filename}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-600">
                    {r.patient?.name || <span className="text-gray-400 italic">Unlinked</span>}
                  </td>
